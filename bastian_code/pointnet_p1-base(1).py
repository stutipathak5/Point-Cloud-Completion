...
log_file_path = os.path.join(log_dir, 'log.txt')  # Full path: "log/log.txt"
os.makedirs(log_dir, exist_ok=True)  # Creates 'log' directory
...
for i in range(300) :
    ...
    log_dir = os.path.join('log', args.log)
    ...
    if i % 15 == 0 :
        ...
        state = {
        'epoch': i + 1, 
        'gen_dict': net.state_dict()
        
        }
        torch.save(state, os.path.join('log/', 'gen_{}.pth'.format(i)))
        # print('saved models')
    ...
    if(save_results): # save all outputs to the save folder
        ...
        if(i%5==0):
            ...
            print("Reconstructed Output Shape:", test_output.shape)
